...
console.log(process.env.STRIPE_SECRECT_KEY);
//
...
//
    ...
    app.get("/manage-orders/:email", async (req, res) => {
      ...
      //customer email ta database e seller ar vitore object hisabe ase tai ''use korsi
      const result = await ordersCollection
        .find({
          "seller.email": email,
        })
  ...
  //  /mongodb teke data anchi
    app.get("/sixBooks", async (req, res) => {
      ...
      const result = await bookCollection.find().limit(6).sort({price: 1 }).project(sixFood)
        .toArray();
        console.log(result);
      res.send(result);
    });
    ...
    app.get("/allBooks", async (req, res) => {
      ...
      const result = await bookCollection.find().sort({price: 1 }).project(sixFood)
        .toArray();
        console.log(result);
      res.send(result);
    });
